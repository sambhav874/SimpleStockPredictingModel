...
st.subheader("Some daily headlines : NYSE and NASDAQ")

def news_nyse():
    main_url="https://newsapi.org/v2/top-headlines?country=us&category=business&apiKey=a52584e6861b41e9a24d1724f00cf267"
    news=req.get(main_url).json()
    ...
    for j in range(10):
        ...
        for arti in article:
            ...
            description_article.append(arti["description"])
            link_article.append(arti["url"])
            
    for i in range(10):
            st.write(i+1,news_article[i])
            st.write(description_article[i])
            st.write(link_article[i])
        
news_nyse()

st.subheader("Some headlines from India: NSE")

def news_nse():
     
    main_url="https://newsapi.org/v2/top-headlines?country=in&category=business&apiKey=a52584e6861b41e9a24d1724f00cf267"
    news=req.get(main_url).json()
    article=news["articles"]
...
def liveData_pred():
    ...
    nMIN=st.slider("Minutes of prediction.",1,10)
...
flag=st.sidebar.selectbox('Select Predictor',['Time Frame Stock Price Predictor','Live Data Stock Price Predictor'])
if flag=='Time Frame Stock Price Predictor':
     timeframe_pred()
else:
     liveData_pred()
